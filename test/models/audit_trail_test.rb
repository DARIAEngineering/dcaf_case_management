  ...
  describe 'methods' do
    ...
    it 'should conveniently render what they were before' do
      assert_equal @track.changed_from,
                   ["Susie Everyteen",
                     "1112223333",
                     (DateTime.now.in_time_zone("EST") + 5.days).strftime('%m/%d/%Y'),
                     (DateTime.now.in_time_zone("EST") + 3.days).strftime('%m/%d/%Y'),
                     "D2-3333"]
    end
    ...
    it 'should conveniently render what they areTime now' do
      assert_equal @track.changed_to,
                  ["Yolo",
                    "1234569999",
                    (DateTime.now.in_time_zone("EST") + 10.days).strftime('%m/%d/%Y'),
                    (DateTime.now.in_time_zone("EST") + 4.days).strftime('%m/%d/%Y'),
                    "D6-9999"]
    end
