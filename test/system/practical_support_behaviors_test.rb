  ...
  describe 'creating a new practical support entry' do
    ...
    it 'should log a new practical support' do
      ...
      within :css, '.modal' do
        ...
        assert_equal 5.days.from_now.strftime('%m/%d/%Y'), find('#practical_support_support_date').value
        assert_equal 6.days.from_now.strftime('%m/%d/%Y'), find('#practical_support_purchase_date').value
